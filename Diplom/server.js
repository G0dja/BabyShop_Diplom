  ...
  '/make-buy-request': async (req, res, url) => {
    try {
      ...
      req.on('end', async () => {
        ...
        if(bodyLength <= 0) throw new Error('Request haven\'t items')
        ...
        for(const id in body) {
          ...
          if(bodyLength !== 1) finalPrice += item.price * item.count
        }

        if(bodyLength !== 1) html += '<h3 style="margin: 0">Остаточна ціна: ' + finalPrice + ' ₴</h3>'
